...
Suite version: `1.2.0`  
Plugin: `com.cjhackeryt.steelseriessonar.macro-deck` `1.0.10`
Conformant: **yes**  
...
| MDC0306 | ui/describe reports a surface list whose every entry names a kind | CapabilitySerialization | Required | SKIP | This subject does not declare the ui capability. |
| MDC0307 | Every surface ui/describe declares names a non-empty session mode | CapabilitySerialization | Required | SKIP | This subject does not declare the ui capability. |
| MDC0308 | ui/describe's reply stays within MaxMessageBytes and deserializes without loss | CapabilitySerialization | Required | SKIP | This subject does not declare the ui capability. |
...
| MDC0802 | Under a logging flood while paused, a trailing Error still survives and Dropped is reported honestly | BoundedQueues | Recommended | FAIL | Expected: An Error logged while draining is paused still reaches the host after draining resumes - the priority channel survives an informational flood.<br>Actual: The action produced 120 log event(s) while draining was paused, but no Error-level event arrived within 5 s of resuming. |
